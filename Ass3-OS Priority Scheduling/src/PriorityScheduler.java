...
class Scheduler {
	...
	void run() {
		...
		while (remainingProcesses != 0) {
			...
			//If current burst time is 0, the process has ended, do the required calculations, add it to the chart,
			//remove it from queue, set getProcess boolean to true to take the upcoming process from the queue
...
public class PriorityScheduler {
	...
	public static void main(String[] args) {
		...
		scanner.nextLine();
		Scheduler priorityScheduler = new Scheduler(contextSwitching);
		...
		for (int i = 0; i < numberOfProcesses; i++) {
			...
			Process newProcess = new Process(processName, arrivalTime, burstTime, priority);
			priorityScheduler.addProcess(newProcess);
		}
		scanner.close();
